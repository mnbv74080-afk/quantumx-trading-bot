...
def send_startup_prices_report():
    """🔔 دالة إرسال تقرير الأسعار المباشرة الفورية فور تشغيل السكربت"""
    print("🔄 جاري تجميع الأسعار الحالية وإرسال تقرير البدء للتليجرام...")
    report_msg = "🚀 **تم تشغيل المحرك بنجاح (XAUUSD PRO - 100K)**\n"
    report_msg += "━━━━━━━━━━━━━━━━━━\n"
    report_msg += "📊 **الأسعار المباشرة الحالية للأصول الـ 10:**\n\n"

    for symbol, config in ASSETS_CONFIG.items():
        try:
            df = yf.download(tickers=config["ticker"], period="1d", interval="1m", progress=False)
            if not df.empty:
                current_price = df['Close'].iloc[-1]
                decimals = 2 if symbol in ['XAUUSD', 'US30', 'NAS100', 'GER30', 'GBPJPY', 'USDJPY'] else 4
                report_msg += f"• `{symbol}`: **{current_price:.{decimals}f}**\n"
            else:
                report_msg += f"• `{symbol}`: ⚠️ جاري الاتصال...\n"
        except Exception as e:
            report_msg += f"• `{symbol}`: ⚠️ خطأ جلب السعر\n"

    report_msg += "\n━━━━━━━━━━━━━━━━━━\n"
    report_msg += "🔍 *جاري الفحص المباشر لاقتناص الفرص واعتماد الصفقات الموصى بها!*"
...
def main():
    ...
    # 🔔 إرسال التقرير الأول للأسعار الفورية بمجرد بدء التشغيل
    send_startup_prices_report()
